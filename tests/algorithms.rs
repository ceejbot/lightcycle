//! Comprehensive evaluation of hash algorithms for rendezvous hashing
//!
//! This test evaluates all available hash algorithms across multiple criteria:
...
use lightcycle::hashes::ConsistentHasher;
use lightcycle::{HasId, HashRing, RendezvousRing};
...
fn evaluate_algorithms() {
    println!("\n=== Hash Algorithm Evaluation for Rendezvous Hashing ===");
    ...
    {
        use lightcycle::hashes::Murmur3Hasher;
    ...
    let fastest = results
        ...
        .min_by(|a, b| {
            a.hash_speed_ns
                .partial_cmp(&b.hash_speed_ns)
                .expect("Hash speed values should be comparable")
        })
        .expect("Results should not be empty");
    println!("Fastest hasher: {} ({:.2} ns/op)", fastest.name, fastest.hash_speed_ns);

    let best_distribution = results
        .iter()
        .min_by(|a, b| {
            a.distribution_chi_squared
                .partial_cmp(&b.distribution_chi_squared)
                .expect("Chi-squared values should be comparable")
        })
        .expect("Results should not be empty");
    println!(
        "Best distribution: {} (χ² = {:.2})",
        best_distribution.name, best_distribution.distribution_chi_squared
    );

    let best_weighted = results
        .iter()
        .max_by(|a, b| {
            a.weighted_accuracy
                .partial_cmp(&b.weighted_accuracy)
                .expect("Weighted accuracy values should be comparable")
        })
        .expect("Results should not be empty");
    println!(
        "Best weighted accuracy: {} ({:.1}%)",
        best_weighted.name, best_weighted.weighted_accuracy
    );
    ...
    let mut scores: Vec<(String, f64)> = results
        .iter()
        .map(|r| {
    ...
    for result in &results {
        assert_eq!(
            ...
            "{} hasher should have perfect consistency",
            result.name
        );
    }
}
